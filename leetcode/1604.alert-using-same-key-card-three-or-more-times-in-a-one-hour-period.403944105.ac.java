...
class Solution {
    public List<String> alertNames(String[] keyName, String[] keyTime) {
        ...
        Arrays.sort(vals, (a, b) -> {
            int v = a[0].compareTo(b[0]);
            return v == 0 ? a[1].compareTo(b[1]) : v;
        });
        List<String> ans = new ArrayList<>();
        for (int i = 2; i < n; i++) {
            if (vals[i][0].equals(vals[i-2][0])) {
                int cur = tm(vals[i][1]) - tm(vals[i-2][1]);
                if (cur <= 60) {
                    ans.add(vals[i][0]);
                    while (i+1 < n && vals[i+1][0].equals(vals[i][0]))
                        i++;
                }
            }
        }
    ...
    int tm(String t) {
        return (t.charAt(0) - '0') * 600 + (t.charAt(1) - '0') * 60 + t.charAt(3) * 10 + t.charAt(4);
    }
}
