...
class Solution {
    ...
    public int minDifficulty(int[] jobDifficulty, int d) {
        ...
        if (n < d)
            return -1;
        cache = new int[d][n];
        return helper(jobDifficulty, 0, 0, d);
    }
    
    int helper(int[] jobDiff, int l, int cur, int d) {
        int n = jobDiff.length;
        if (cur == d || l == n) {
            return l < n ? 10000000 : 0;
        }
        if (cache[cur][l] > 0)
            return cache[cur][l];
        int max = 0;
        int ans = 10000000;
        for (int i = l; i + (d-cur-1) < n; i++) {
            max = Math.max(max, jobDiff[i]);
            ans = Math.min(ans, max+helper(jobDiff, i+1, cur+1, d));
        }
        return cache[cur][l] = ans;
    }
}
