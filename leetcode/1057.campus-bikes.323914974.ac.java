/*
 * @lc app=leetcode id=1057 lang=java
 *
 * [1057] Campus Bikes
 *
...
class Solution {
    public int[] assignBikes(int[][] workers, int[][] bikes) {
        List<int[]>[] pqs = new List[2001];
        for (int i = 0; i < workers.length; i++)
            for (int j = 0; j < bikes.length; j++) {
                int len = Math.abs(workers[i][0] - bikes[j][0]) + Math.abs(workers[i][1] - bikes[j][1]);
                if (pqs[len] == null) {
                    pqs[len] = new ArrayList<int[]>();
                }
                pqs[len].add(new int[] {i, j});
            }
        boolean[] wf = new boolean[workers.length];
        boolean[] bf = new boolean[bikes.length];
        int[] ans = new int[workers.length];
        int cnt = 0;
        for (List<int[]> pq : pqs) {
            if (pq != null) {
                for (int[] arr : pq) {
                    if (wf[arr[0]] || bf[arr[1]])
                        continue;
                    ans[arr[0]] = arr[1];
                    wf[arr[0]] = true;
                    bf[arr[1]] = true;
