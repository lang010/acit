...
class Solution {
    public int numOfMinutes(int n, int headID, int[] manager, int[] informTime) {
        int min = 0;
        for (int i = 0; i < n; i++) {
            cnt(i, manager, informTime);
            min = Math.min(min, informTime[i]);
        }
        return -min;
    }
    
    int cnt(int x, int[] manager, int[] informTime) {
        if (informTime[x] < 0)
            return informTime[x];
        if (manager[x] < 0) {
            informTime[x] = -informTime[x];
        } else {
            informTime[x] = cnt(manager[x], manager, informTime) - informTime[x];
        }
        return informTime[x];
    }
}
