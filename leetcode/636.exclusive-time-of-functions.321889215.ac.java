...
class Solution {
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        for (String s : logs) {
            String[] ls = s.split(":");
            int curId = Integer.parseInt(ls[0]);
            int curTm = Integer.parseInt(ls[2]);
            if ("start".equals(ls[1])) {
                if (!stack.isEmpty()) {
                    ans[stack.peek()] += curTm - preTm;
                }
                stack.add(curId);
            } else if ("end".equals(ls[1])) {
                curTm++;
                ans[curId] += curTm - preTm;
                stack.pop();
            }
            preTm = curTm;
        }
