 ...
 * box, then that box and all other boxes behind it will be stopped before that
 * room.
 * 
 * 
...
class Solution {
    public int maxBoxesInWarehouse(int[] boxes, int[] warehouse) {
        ...
        Arrays.sort(boxes);
        int b = 0, w = warehouse.length-1;
        while (b < boxes.length && w >= 0) {
            while (w >= 0 && warehouse[w] < boxes[b])
                w--;
            if (w >= 0) {
                w--;
                b++;
