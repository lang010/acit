...
class Solution {
    ...
    public int unhappyFriends(int n, int[][] pref, int[][] pairs) {
        ...
        for (int i = 0; i < pairs.length; i++)
            for (int j = i+1; j < pairs.length; j++)
                cnt(pairs[i][0], pairs[i][1], pairs[j][0], pairs[j][1]);
        int sum = 0;
        for (int x : ans)
            sum += x;
        return sum;
    ...
    int cnt(int x, int y, int u, int v) {
        return cntr(x, y, u, v) + cntr(u, v, x, y);
    }
    
    int cntr(int x, int y, int u, int v) {
        return cn(x, y, u, v) + cn(y, x, u, v) + cn(x, y, v, u) + cn(y, x, v, u);
    }
    
    int cn(int x, int y, int u, int v) {
