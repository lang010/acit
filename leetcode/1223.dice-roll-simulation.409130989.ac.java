...
class Solution {
    ...
    public int dieSimulator(int n, int[] rollMax) {
        ...
        int ans = 0;
        for (int i = 0; i < 6; i++)
            ans = (ans + dfs(n, i, 1, rollMax))%mod;
        return ans;
    }
    
    int dfs(int n, int cur, int pre, int[] max) {
        if (n == 1)
            return 1;
        if (cache[n-1][cur][pre-1] > 0)
            return cache[n-1][cur][pre-1];
        int ans = 0;
        if (pre < max[cur])
            ans = dfs(n-1, cur, pre+1, max);
        for (int i = 0; i < 6; i++)
            if (cur != i)
                ans = (ans + dfs(n-1, i, 1, max))%mod;
        return cache[n-1][cur][pre-1] = ans;
    }
}
